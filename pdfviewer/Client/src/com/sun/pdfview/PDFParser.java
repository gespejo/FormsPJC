/*
 * $Id: PDFParser.java,v 1.6 2009/01/16 16:26:12 tomoke Exp $
 *
...
public class PDFParser extends BaseWatchable {
    ...
    private String readName() {
        int start = loc;
        while (loc < stream.length && !PDFFile.isDelimiter(stream[loc])) {
            loc++;
        }
    ...
    /**
     * read a String from the stream.  Strings begin with a '('
    ...
    private PDFObject findResource(String name, String inDict)
        ...
        if (inDict != null) {
            ...
            return (PDFObject) in.getDictRef(name);
        } else {
    ...
    private void parseInlineImage() throws IOException {
        ...
        // for some totally undocumented reason, inline image-masks seem to
        // use the opposite meanings for 0 and 1 than the rest of the world.
        // In order to make them work right we do one of two things:
        // if there is no decode array, add a [1 0] decode array.  If there is
        // a decode array, swap the values in it.  It's weird, but that's
        // the only way I can figure to get them not inverted.
        PDFObject imObj = (PDFObject) hm.get("ImageMask");
        if (imObj != null && imObj.getBooleanValue()) {
            // the default (if there is no decode array)
            Double[] decode = {new Double(1), new Double(0)};

            // if there is a decode array, invert it
            PDFObject decodeObj = (PDFObject) hm.get("Decode");
            if (decodeObj != null) {
                decode[0] = new Double(decodeObj.getAt(1).getDoubleValue());
                decode[1] = new Double(decodeObj.getAt(0).getDoubleValue());
            }
